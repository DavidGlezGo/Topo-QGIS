...
from qgis.core import QgsProcessingAlgorithm, QgsProcessingParameterRasterLayer, QgsProcessingParameterRasterDestination, QgsProcessingParameterEnum
from .. import topopy
...
class Get_Orders(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        """
        self.addParameter(QgsProcessingParameterRasterLayer(self.INPUT_FD,  self.tr("Input Flow")))
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        dem = DEM(input_fd.source())
        fd = Flow(dem)
        nt = Network(fd)
        if order == 0:
            outo = nt.get_stream_orders()
           
        if order == 1:
            outo = nt.get_stream_orders('shreeve')
